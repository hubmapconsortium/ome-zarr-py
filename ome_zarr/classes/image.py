...
class OMEZarrMultiscaleBase:
    ...
    def from_ome_zarr(
        ...
        for dataset in metadata.datasets:
            path = dataset.path
            try:
                target_node = group[path]
            except KeyError:
                group_inner_path = metadata.name.lstrip("/")
                actual_array_path = f"{group_inner_path}/{path}"
                target_node = group[actual_array_path]
            data = da.from_zarr(target_node)
